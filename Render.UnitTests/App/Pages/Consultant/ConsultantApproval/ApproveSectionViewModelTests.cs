...
namespace Render.UnitTests.App.Pages.Consultant.ConsultantApproval
{
	public class ApproveSectionViewModelTests : ViewModelTestBase
	{
		...
		public ApproveSectionViewModelTests()
		{
			...
			MockContextProvider.Setup(x => x.GetSnapshotService()).Returns(_mockSnapshotService.Object);
		}
		...
		public async Task TryApprove_Succeeds()
		{
			...
			var mockSectionMovementService = new Mock<ISectionMovementService>();
			...
			MockContextProvider.Setup(x => x.GetSectionMovementService()).Returns(mockSectionMovementService.Object);
			...
			mockSectionMovementService.Verify(x => x.AdvanceSectionAsync(It.IsAny<Section>(),
				It.IsAny<Step>(), It.IsAny<Guid>(), It.IsAny<Guid>()), Times.Once);
		...
		public async Task TryApprove_SetScopeEndDate_Succeeds()
		{
			//Arrange
			var mockSectionRepository = new Mock<ISectionRepository>();
			var sectionMovementService = new Mock<ISectionMovementService>();
			MockContextProvider.Setup(x => x.GetSectionMovementService()).Returns(sectionMovementService.Object);
			MockStageService.Setup(x => x.StepsAssignedToUser())
				.Returns(new List<Guid>());
			MockStageService.Setup(x => x.SectionsAtStep(It.IsAny<Guid>()))
				.Returns(new List<Guid>());
		...
		public async Task ReturnSection_ConsultantRevise__Succeeds()
		{
			...
			var consultantCheckStage = ConsultantCheckStage.Create();
			...
			workflow.AddStage(consultantCheckStage);
			mockWorkflowService.Setup(x => x.ProjectWorkflow)
				.Returns(workflow);
			MockStageService
				.Setup(x => x.GetStepToWorkAsync(It.IsAny<Guid>(), RenderStepTypes.ConsultantApproval, It.IsAny<Guid>()))
			...
			// SetupMocksForHomeViewModelNavigation method setups mocked StageService with mocked workflow
			// this is to override mocked StageService with actual workflow for testing
			...
			_mockSnapshotService.Verify(x => x.ReturnBack(
				It.IsAny<List<Stage>>(),
				It.IsAny<Section>(),
				It.IsAny<Stage>(), 
				It.IsAny<Stage>(),
