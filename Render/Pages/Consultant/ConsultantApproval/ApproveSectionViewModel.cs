...
namespace Render.Pages.Consultant.ConsultantApproval
{
	public class ApproveSectionViewModel : WorkflowPageBaseViewModel
	{
			...
			Guid currentProjectId)
		{
			...
			var step = await stageService.GetStepToWorkAsync(sectionId, RenderStepTypes.ConsultantApproval, currentProjectId);
		...
		public async Task<DialogResult> ShowConfirmationModal()
		{
			var modalService = ViewModelContextProvider.GetModalService();
			...
			return await modalService.ConfirmationModal(confirmationModal);
		}
		...
		public async Task NavigateToSelectApprove()
		{
			...
			await workflowService.RemoveHoldingTankSectionStatusesAfterApproval(Section.Id);
		...
		public async Task ReturnSectionBackAndGoHOme()
		{
			...
			await _snapshotService.ReturnBack(allStages, Section, Stage, previousStage, previousStep, Consultant.Id);

			LogAndRemoveSectionFromApproveList();
		}

		private void LogAndRemoveSectionFromApproveList()
		{
