...
public class ConsultantCheckSectionSelectViewModel : WorkflowPageBaseViewModel
{
    ...
    [Reactive]
    public bool ReviewedSelected { get; set; }

    [Reactive] 
    public ReactiveCommand<Unit, Unit> SelectUnreviewedCommand { get; set; }

    [Reactive]
    public ReactiveCommand<Unit, Unit> SelectReviewedCommand { get; set; }
        ...
        Step step)
    {
        ...
        unreviewedSectionIds.AddRange(stageService.SectionsAtStep(step.Id));
        ...
        if (reviseSteps != null)
        {
            foreach (var reviseStep in reviseSteps)
            {
                ...
                var sectionIds =
                    stageService.GetAllAssignedSectionAtStep(reviseStep.Id, reviseStep.RenderStepType);
        ...
        var unreviewedSections = allSections
            .Where(x => unreviewedSectionIds.Contains(x.Id))
            .OrderBy(x => x.Number)
            .ToList();
        var reviewedSections = allSections
            .Where(x => reviewedSectionIds.Contains(x.Id))
            ...
            secondPageName: AppResources.SectionSelect)
    {
        ...
        if (color != null)
        {
            TitleBarViewModel.PageGlyph = IconExtensions.BuildFontImageSource(Icon.ConsultantCheckOriginal, color.Color, 35)?.Glyph;
        }
