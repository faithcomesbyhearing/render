...
namespace Render.Services.SessionStateServices
{
	public class SessionStateService : ISessionStateService
	{
		...
		public void StartPage(Guid sectionId, string pageName, PassageNumber passageNumber, Guid nonDraftTranslationId, Step step = null)
		{
			...
			//If we're just going to or from the Drafting page in the current step/section
			//(e.g. going from drafting to section listen, where we don't have passageNumber)
			//then need to save draft audios(but need to check whether this passage isn't divided)
			if (ActiveSession.SectionId == sectionId &&
				(ActiveSession.PageName.Contains("Drafting") || pageName.Contains("Drafting")) &&
				ActiveSession.NonDraftTranslationId == nonDraftTranslationId &&
				PassageNumber.BothAreNotNullAndNotEqual(ActiveSession.PassageNumber, passageNumber) is false)
			{
		...
		public void SetCurrentStepForDraftOrRevise(Guid stepId, Guid sectionId)
		{
			ActiveSession = AllSessions.FirstOrDefault(x => x.StepId == stepId);
			...
			if (ActiveSession.SectionId == default && ActiveSession.DraftAudioIds.IsNullOrEmpty())
			{
				ActiveSession.Reset();
			}
		}
